...
// Calc_TM_Score_Simple seems not to be called
							   ...
							   double *MAXSCO)
{
	...
	//make n_init = i+1 the exponent that 2^n_init >= ali_orin
	for(i=0;i<n_init_max-1;i++)
	{
		L_ini[i]=(int)(1.0*ali_orin/(exp(1.0*i*log(2.0))));
		// cout << "L_ini " << i << ": " << L_ini[i] << endl;
...
tag_402:
	...
	for(i_init=0;i_init<n_init;i_init++)
	{
		...
		// SEARCHorNOT makes the step be 40 instead of 1
		if(SEARCHorNOT==1)
		{
			ws=0;
...
/*
		//one time for a given alignment
